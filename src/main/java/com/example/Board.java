...
public class Board extends JPanel implements MouseListener, MouseMotionListener {
                ...
                if (s.isOccupied()) {
                    ...
                    if (p.getColor() != whiteTurn) {
                        for (int i = 0; i < controlled.size(); i++) {
                            ...
                            if (s2.isOccupied() && (s2.getOccupyingPiece() instanceof King)
                                    && (s2.getOccupyingPiece().getColor() == whiteTurn)) {
                                isInCheck = true;
                            }
                        }
                    }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (fromMoveSquare != null)
            fromMoveSquare.setBorder(BorderFactory.createMatteBorder(1, 1, 1, 1, Color.BLUE));
    ...
    // Post-condition: If the user moved a piece to a valid square in a legal
    // manner, the selected piece shall move to the selected square. The turn will
    // also change if the move legal
    @Override
    public void mouseReleased(MouseEvent e) {
        Square endSquare = (Square) this.getComponentAt(new Point(e.getX(), e.getY()));

        if (fromMoveSquare != null && currPiece != null) { // Move only to square if there actually is a square and the
                                                           // user is actually holding a piece
            if (currPiece.getLegalMoves(this, fromMoveSquare).contains(endSquare)) { // Move only if the end square can
                                                                                     // legally be moved to by the piece
                        ...
                        if (currPiece instanceof King) {
                            ...
                            if (board[endSquare.getRow() + vertical][endSquare.getCol()].isOccupied()) {
                                if (board[endSquare.getRow() + vertical][endSquare.getCol()]
                                        .getOccupyingPiece() instanceof Pawn) {
    ...
    private void endTurnStuff(Square endSquare) {
        whiteTurn = !whiteTurn;
        ...
        if (currPiece instanceof Pawn) {
            if (endSquare.getRow() == 0)
                endSquare.put(new Queen(true, RESOURCES_WQUEEN_PNG));
            if (endSquare.getRow() == 7)
                endSquare.put(new Queen(false, RESOURCES_BQUEEN_PNG));
        }
